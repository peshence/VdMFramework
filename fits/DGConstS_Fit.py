...
class DGConstS_Fit(FitManager.FitProvider):
    ...
    def doFit( self,graph,config):
        ...
        Sigma = config['Sigma']
        SigmaErr = config['SigmaErr']
        LimitSigma_lower = Sigma
        LimitSigma_upper = Sigma
        ...
        ff.SetParameters(Sigma, 1.,StartPeak,StartFrac,0.,StartConst)
        ...
        if LimitConst_upper > LimitConst_lower:
            ff.SetParLimits(4, LimitConst_lower,LimitConst_upper)
...
#        peakErr = math.sqrt(constErr*constErr+ampErr*ampErr)
        ...
        sigma = CapSigma / sqrttwopi
        sigmaErr = CapSigmaErr / sqrttwopi
# ----
        ...
        self.table.append([scan, type, bcid, sigma, sigmaErr, sigRatio, sigRatioErr, frac, fracErr, mean, meanErr, const, constErr, CapSigma, CapSigmaErr, peak, peakErr, area, areaErr, fitStatus, chi2, ndof])
