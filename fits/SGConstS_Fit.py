...
class SGConstS_Fit(FitManager.FitProvider):

    fitDescription = """ Single Gaussian with const background and set CapSigma.
    ...
    def doFit( self,graph,config):
        ...
        ff = r.TF1("ff","[2] + [1]*exp(-(x-[0])**2/(2*"+ str(StartSigma)+"**2))")
        ff.SetParNames("#Sigma","Mean","peak","Const")

        ff.SetParameters(0.,StartPeak, StartConst)
