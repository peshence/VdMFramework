...
corrected = '/brildata/vdmoutput/AutomationSingelEff/Analysed_Data/'
...
def plotfit(x,y,goodname):
    ...
    la = round(la,len(str(lae)) - str(lae).index('.') - 1)
    lb = round(lb,len(str(lbe)) - str(lbe).index('.') - 1)
    plot.plot(x, [la*i + lb for i in x], label = '(' + str(la) + '$\pm$' + str(lae) + ')x + ' + str(lb) + '$\pm$' + str(lbe))
    plot.ylabel('$\Delta ' + goodname + '$ [%]')
    plot.xlabel('SBIL')
...
def do(goodname,detector,v):
    ...
    for fol in os.listdir(corrected):
        ...
        capx = [(i-j)/i * 100 for i,j in zip(fitparams.CapSigma[:len(lumi.xsec)-1], fitparamscorr.CapSigma[:len(lumi.xsec)-1])]
        capy = [(i-j)/i * 100 for i,j in zip(fitparams.CapSigma[len(lumi.xsec)-1:], fitparamscorr.CapSigma[len(lumi.xsec)-1:])]
        ...
        def plotthishit(varname, goodname, coord):
            ...
            plot.title(detector + ' effect of nonlinearity on $\Delta ' + goodname + '$ \n' + fill)
