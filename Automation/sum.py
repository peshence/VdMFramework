...
for detector in detectors:
    ...
    for scan in scans:
        ...
        def plotVar(plt,df,varname,corrange):
            ...
            if tslopex and tslope and len(tslope)!=0 and len(tslopex)!=0:
                ...
                print fill, varname, ' lead', np.polyfit(tslopex + lslopex,lslope,1)
            
            def round_to_sign(x):
                return round(x, -int(np.floor(np.log10(np.abs(x))))+1)
            ae = round_to_sign(cov[0][0])
            be = round_to_sign(cov[1][1])
            a = round(a,len(str(ae)) - '{0:f}'.format(ae).index('.') - 1)
            b = round(b,len(str(be)) - '{0:f}'.format(be).index('.') - 1)
    ...
    for k in globalSlope.keys():
        (a,b),cov = np.polyfit(globalSlope[k]['x'], globalSlope[k]['y'],1, cov = True)
        ...
        plot.plot(globalSlope[k]['x'], [a*i + b for i in globalSlope[k]['x']], label = 'fit all ' + str(a) + '$\pm$' + str(ae) + 'x + ' + str(b) + '$\pm$' + str(be))
