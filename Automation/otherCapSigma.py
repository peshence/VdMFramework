...
for corr in corrs:
    for scan in scans:
        ...
        for det in os.listdir(analysisdir + scan):
            ...
            curdf = pd.DataFrame.from_csv(analysisdir + scan + '/' + det + '/results/' + corr + '/' + fit(det,scan) + '_FitResults.csv')#.loc[:,['CapSigma','CapSigmaErr','peak','peakErr']]
    ...
    pd.DataFrame.from_dict(dav).to_csv('avBCapsigma_' + corr + '.csv')