...
def doMakeRateFile(ConfigInfo):
    ...
    with tables.open_file(InputLumiDir, 'r') as h5file:
        beamtable = None
        for table in h5file.root:
            if table.name == RateTable:
                beamtable = table
        ...
        if not beamtable:
            print 'No table name ' + rateTable + ' in this file - ' + filename
        collBunches = np.nonzero(h5file.root.beam[0][19]*h5file.root.beam[0][20])[0]
        iterator = iter([r for  r in beamtable[:]])
        row = next(iterator)
        ...
        for i in range(len(ScanNames)):
            ...
            for j, sp in enumerate(scanpoints):
                ...
                temprates = []
                while row['timestampsec']>sp[3]:
                    row = next(iterator)
                while row['timestampsec'] <= sp[4]:
                    temprates.append(row)
                
                avgdf = pd.DataFrame([i['avgraw'] for i in temprates])
                bxdf = pd.DataFrame([i['bxdata'] for i in temprates])
                if bxdf.empty:
                    print "Attention, rates bxdf empty because timestamp window not contained in file"
                else:
                    for idx, bcid in enumerate(collBunches):
