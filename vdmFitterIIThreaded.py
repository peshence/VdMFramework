...
def doRunVdmFitter(Fill, FitName, InputGraphsFiles, OutputDir, PlotsTempPath, FitConfigInfo):
    ...
    if (len(InputGraphsFiles)==1):
     for keyAll in sorted(graphsAll_dict.keys()):
        ...
        pool = len(keys)/15
        print keys, pool
        def ThreadedFit(q,keys, FitName,i):
            results = []
            fitter = availableFits[FitName+'_Fit']()
            for key in orderedIntKeysFirst(keys):
                # print "------>>>>"
                ...
                # else:    
                result = fitter.doFit(graph, FitConfigInfo, key,i)
                #results[key] = result
                functions = result[0]
                canvas = fitter.doPlot(graph, functions, Fill, PlotsTempPath[0])
                q.put(result,fitter.table)
        
        threads = []
        def wrap(keys, FitName,i):
            q = Queue.Queue()
            thread = threading.Thread(target=ThreadedFit, args=(q,keys[i*pool:(i+1)*pool], FitName,i))
            thread.start()
            thread.result_queue = q
            return thread
            
        resultslist = [{} for i in range(16)]
        tablelist = [[] for i in range(16)]
        table = []
        table.append(["Scan", "Type", "BCID", "sigma","sigmaErr", "sigmaRatio","sigmaRatio_Err", \
                      "Frac","FracErr","Mean","MeanErr", "CapSigma", "CapSigmaErr", "peak", "peakErr", \
                      "area", "areaErr","fitStatus", "chi2", "ndof"])
        for i in range(16):
            # print keys,i*pool,(i+1)*pool,resultslist
            # print keys[i*pool:(i+1)*pool], resultslist[i]
            # input()
            thread =  wrap(copy.deepcopy(keys[i*pool:(i+1)*pool]), FitName,i)
            threads.append(thread)
            # thread = threading.Thread(target=ThreadedFit, args=(copy.deepcopy(keys[i*pool:(i+1)*pool]), FitName))
            # thread.start()
            # threads.append(thread)
        ...
        while threading.active_count >0:
            ...
            time.sleep(10)
        for i in range(16):
            resultslist[i], tablelist[i] = threads[i].result_queue.get()            
            results.update(resultslist[i])
            table.append(tablelist[i])
...
if __name__ == '__main__':
    ...
    if 'InputGraphsFile' in ConfigInfo:
        InputGraphsFile = AnalysisDir + '/' + Luminometer + '/' + ConfigInfo['InputGraphsFile']
    ...
    if 'Sim' in FitConfigFile:
        ...
        if 'InputSimGraphsFile' in ConfigInfo:
            InputSimGraphsFile = AnalysisDir + '/' + 'VTX' + '/' + ConfigInfo['InputSimGraphsFile']
